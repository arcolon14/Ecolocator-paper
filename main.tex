...
and they are vulnerable to the neutral allele frequency gradients
described above
...
and therefore cannot predict an individual's climate of origin.
...
We evaluate \ecolocator across a range of simulations
...
This combination of strong local adaptation and a well-developed
biological and management framework
makes \dfir a natural bridge
between controlled simulations and real-world management applications:
seed zones give us an independent, management-relevant standard
against which to judge \ecolocator predictions.
...
Geographic and environmental prediction are decoupled: 
any accuracy \ecolocator achieves on the
...
In the second, QTLs were distributed across three 100 kb windows.
In the third, these smaller windows were spread across five windows
...
and recapitated using pyslim 1.0.4 \citep{haller2019treeseq}.
...
In each parameterization test case,
we held the other parameters at an intermediate value
(i.e., when testing dispersal parameters,
we held \(\sigma_K\) at 0.5).
Spatial and phenotypic scales in these simulations are defined relative to the simulated landscape
and environmental gradient rather than to physical distances,
...
and the three genomic architectures distribute adaptive variation
differently along the chromosome,
from a single 200 kb QTL window
to three or five 100 kb windows
spaced evenly across the 1 Mb element.
...
we applied SHapley Additive exPlanations (SHAP) using the Python \texttt{SHAP} package (\texttt{v.$\geq$0.47.0})
\citep{lundberg_shap_2017},
which is based on the game-theoretic framework that assigns importance scores
...
stratified by LD strength (\Cref{fig:permutation}).
...
\citet{booker_wza_2024} showed that \texttt{WZA} and the top-candidate method
performed as well as or better than a range of single-SNP methods,
including Kendall's $\tau$, \texttt{BayPass}, \texttt{LFMM}, and \texttt{RDA},
across both strong and weak local adaptation scenarios.
We therefore focus on \texttt{WZA} and the top-candidate method
...
\texttt{WZA} aggregates SNP-level signals
...
after filtering. The \dfir genome is large and incompletely
...
pollen donors. While the pollen pool is heterogeneous, pollen and seed movement in \dfir
...
Genotypes, coordinates, and climate values were preprocessed as described above,
with coordinates and covariates centered and scaled prior to training.
...
To test whether climate predictions carry information beyond geography,
we also derived indirect climate estimates by looking up each
\ecolocator-predicted location in ClimateNA
and compared their accuracy to the direct climate predictions.
...
Because the SNP array carries no chromosome coordinates,
we additionally aligned array probe sequences to a recent
...
These positions were used only to place attribution scores in genomic context,
not for prediction.
...
For each tree, we compared \ecolocator climate predictions to seed zone climates
using two approaches.
In the ``fixed-zone'' approach, map-based seed zones were treated as fixed constraints:
a prediction is a ``match'' if it falls within
the percentile-defined climate range of the source seed zone and elevation band.
This map-based approach can produce non-matches when predictions are nearly identical
to true values, because fixed map boundaries are arbitrary.
In the ``focal point'' approach, the climate range of the source seed zone
defines the maximum acceptable climate transfer:
a prediction is a match if its distance from the true value
is smaller than the climate range of the source seed zone--elevation band.
For example, if a source seed zone--elevation band shows a
MCMT 100th percentile range of 2\textdegree{}C, an \ecolocator MCMT prediction
within 2\textdegree{}C of the true value is a match.
Under both approaches we used the 100th percentile range
(defined by the minimum and maximum values)
and the 98th percentile range (defined by the 1st and 99th percentile values),
and we applied the same criteria to the indirect (location plus ClimateNA) estimates.
...
(d = 0.1, Lon. $R^2$=0.968, Lat. $R^2$=0.967, 
Env. $R^2$=0.769) (\Cref{fig:sim-performance}, panel a),
while increasing dispersal reduced accuracy
for both geographic and environmental prediction 
(d=0.5, Lon. $R^2$=0.815, 
Lat. $R^2$=0.814, Env. $R^2$=0.336) (\Cref{fig:sim-performance}, panel a).
...
(\(\sigma_K\)=0.3, Lon. $R^2$=0.917, 
Lat. $R^2$=0.916; \(\sigma_K\)=0.5, Lon. $R^2$=0.911, 
Lat. $R^2$=0.911; \(\sigma_K\)=0.7, 
Lon. $R^2$=0.906, Lat. $R^2$=0.907).
This dissociation is itself informative: environmental accuracy declined as selection weakened,
whereas geographic accuracy remained stable because dispersal,
and therefore the spatial structure of the populations,
was held constant.
Together with the spatially uncorrelated landscape (H=0),
which prevents geography from directly predicting the environmental target,
this result supports that \ecolocator can recover
genotype--environment signal that is distinguishable from neutral geographic structure.
Varying genetic architecture produced only a slight difference in local adaptation
($\vec{\Delta}$LA between GA1 and GA3 = 0.008),
...
Per-replicate LOOCV $R^2$ again declined with 
...
where restricted migration among neighboring demes
generates strong spatial structure and local adaptation is driven
by a small number of causal loci with localized genotype--environment associations.
...
(\Cref{fig:gea-comparison}). The random baseline represents the AUC-PR expected if variants were ranked at random,
...
substantially exceeding \texttt{WZA} (0.079)
...
(\Cref{fig:gea-comparison}, panel d).
...
and \texttt{WZA} in the Wright--Fisher simulations
...
we applied \ecolocator to 348 coastal \dfir trees genotyped at 18,732 SNPs
and sampled across Oregon and Washington (\Cref{fig:dfir-results}, panel f),
predicting geographic origin together with three climate variables
that structure local adaptation in this species: MCMT, SHM, and MSP.
Leave-one-out cross-validation gave a predicted and a known value
for every tree and target.
...
\begin{figure}[H]
    ...
    Mean Cold Month Temperature (MCMT) ($R^2$= 0.72), Summer Heat:Moisture Index (SHM) 
    ($R^2$= 0.52), and Mean Summer Precipitation (MSP) ($R^2$= 0.54). \textbf{Panels f-g.}
...
Because climate is strongly spatially autocorrelated,
climate predictions could in principle ride entirely on geographic prediction.
If so, the indirect route---predicting location and then looking up its
climate in ClimateNA---would do at least as well as direct prediction.
Instead, direct climate estimates from \ecolocator
were more accurate than ClimateNA values at predicted locations
for all three variables (\Cref{tab:supptable1}),
indicating that \ecolocator recovers climate-related genomic signal
beyond what geography alone supplies. Among the three modeled climate variables,
...
we compared predicted climate values to the climate ranges
within first-generation USFS seed zone--elevation bands,
...
For the subset of SNPs with approximate chromosomal positions
recovered from the DFv5.29 draft assembly,
plotting mean $\lvert\mathrm{SHAP}\rvert$ against position
...
MSP and SHM both capture variation related to summer moisture availability and showed 
the strongest overlap among high-attribution SNPs. Similarly, 
MCMT varies strongly along the longitudinal transition from relatively mild 
maritime environments toward colder environments closer to the Cascade Range 
...
Varying genetic architecture allowed us to test whether
\ecolocator's performance depends on how adaptive effects
are distributed across the genome.
Despite differences in the number and distribution of adaptive loci,
prediction accuracy remained largely unchanged across the three
architectures tested.
This suggests that \ecolocator can recover geographic
and environmental signal across both more concentrated
and more distributed architectures,
although the range explored here represents only a subset
...
(Lon. $R^2$=0.75, Lat. $R^2$=0.83, MCMT $R^2$=0.72, MSP $R^2$=0.54, SHM $R^2$=0.52), 
...
One limitation, however, is particularly important when 
extending feature attribution from simulations to empirical landscapes. 
Geography and climate frequently covary, such that variants associated 
with spatial population structure may also appear associated with 
environmental gradients even when they are not themselves targets of 
climate-mediated selection. This challenge is shared by conventional 
GEA approaches \citep{booker_wza_2024, meirmans_ibd_2012}. 
Our continuous-space simulations largely avoid this problem by design, 
holding climate independent of geography ($H=0$) so that genotype--climate 
signal can be evaluated against known causal loci (\Cref{fig:shap-attribution}). 
In empirical systems where geography and environment covary, attribution alone 
cannot establish whether a high-attribution locus reflects direct environmental 
selection, demographic structure, linkage to a selected locus, or some 
combination of these processes. High-attribution loci should therefore be 
interpreted as candidate genomic regions associated with the predicted 
gradient rather than as confirmed causal targets of adaptation.

The \dfir results nevertheless provide additional information 
about how this signal is structured. Overlap among high-attribution SNPs 
was greatest between prediction targets that represent closely related 
environmental or geographic gradients: MSP and SHM shared 74.3\% of their 
highest-attribution loci, while longitude and MCMT shared 54.2\%. 
In contrast, latitude and MCMT shared only 7.3\% (\Cref{tab:dfir-shap-overlap}). 
This pattern suggests that \ecolocator is not assigning uniformly high 
attribution to the same spatially structured loci across all prediction targets, 
but instead recovers partially target-specific genomic signal corresponding 
to different axes of geographic and environmental variation. 
Functional annotations provide complementary biological support for this 
interpretation. Latitude-associated candidates represented light sensing 
and seasonal developmental regulation, consistent with known effects of 
photoperiod and temperature on seasonal growth 
cessation in coastal \dfir \citep{ford_photoperiod_2017}.
Longitude and MCMT candidates included functions related to seasonal 
regulation and temperature acclimation, consistent with previous genomic 
evidence for polygenic cold adaptation involving growth, phenology, 
and stress signaling in \dfir \citep{delatorre_polygenic_2021}. 
MSP and SHM candidates repeatedly represented ABA-mediated water-stress signaling, 
osmotic protection, and redox response, while previous common-garden work in coastal 
\dfir has similarly demonstrated climate-related genetic variation in drought 
and cold hardiness across winter-temperature and summer-precipitation 
gradients \citep{bansal_tolerance_2016}. Viewed together, 
...
The approximate chromosomal coordinates recovered by aligning probe
sequences to the DFv5.29 draft assembly were used only to place SHAP
attributions in genomic context (\Cref{fig:shap-manhattan-dfir}),
not for prediction.
...
prediction tasks. However, when geographic and environmental gradients 
are strongly correlated, shared genomic signal between these tasks cannot 
necessarily be attributed uniquely to demographic structure or local adaptation.
