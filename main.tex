...
Because dispersal, and therefore spatial structure, was held constant
across selection regimes, and because the H=0 landscape prevents geography
from predicting the environmental target,
this dissociation shows that \ecolocator recovers
genotype--environment signal that geographic structure alone does not supply.
...
Lat. $R^2$=0.985,
Env. $R^2$=0.916).
This regime combines strong spatial structure from neighbor-limited migration
with local adaptation driven by a small number of causal loci,
which explains the high accuracy here and, as shown below,
the stronger attribution and GEA performance in the same simulations.
...
In our simulations, dispersal degraded both geographic and environmental
prediction, whereas selection strength affected only environmental prediction.
...
Prediction accuracy was robust to the three genetic architectures tested,
although this range represents only a subset
...
Discrete demes, neighbor-limited migration,
and directional selection on a small number of causal loci
create a clearly structured prediction task,
whereas continuous space, diffuse genetic architectures,
high dispersal, or weak selection weaken the signals
that \ecolocator is designed to learn.
The contrast between the two regimes shows that prediction difficulty
depends on the structure of the evolutionary problem,
...
about how this signal is structured. High-attribution SNPs overlapped
most between targets that represent closely related gradients
(MSP and SHM; longitude and MCMT) and least between unrelated ones
(latitude and MCMT; \Cref{tab:dfir-shap-overlap}).
\ecolocator therefore is not assigning uniformly high
attribution to the same spatially structured loci across all prediction targets,
but recovers partially target-specific genomic signal corresponding
to different axes of geographic and environmental variation.
The functional annotations support this interpretation and agree with
prior work in coastal \dfir: light sensing and seasonal developmental
regulation among latitude candidates match known effects of photoperiod
and temperature on growth cessation \citep{ford_photoperiod_2017};
seasonal regulation and temperature acclimation among longitude and MCMT
candidates match genomic evidence for polygenic cold adaptation
\citep{delatorre_polygenic_2021}; and ABA-mediated water-stress signaling,
osmotic protection, and redox response among MSP and SHM candidates match
common-garden evidence for genetic variation in drought and cold hardiness
along summer-precipitation and winter-temperature gradients
