...
% ARC - You have some of the software names formatted below so I
% added it here as well. I like having them formatted differently
% from the "regular" text as well.
...
% ARC - Just trying to break down the longer sentence
...
\texttt{SHAP} \citep{lundberg_shap_2017}, % First time mentioning SHAP so consider movinb the whole name here
...
between controlled simulations and real-world management applications. % ARC - I think this should be the last sentence of the paragraph. Maybe moving the next one about the seed zones up and then finishing with this summary about Dfir.
...
(e.g., VCF, Zarr, tab-delimited matrices) as input,
...
experiencing local adaptation of a quantitative trait % ARC - of a quantitative trait? from a quantiative trait?
...
% ARC - changed this to have a different sentence structure from the
% one before.
...
across three genetic architectures. In the first,
all QTLs were clustered in a single 200 kb window
\sout{(bases 400,000--599,999; 20\% of the chromosome),} centered at its midpoint.
In the second,
QTLs were split across three evenly-spaced 100 kb windows (10\% of the chromosome each, 30\% total),
\sout{centered at bases 250,000, 500,000, and 750,000
(the 25\%, 50\%, and 75\% positions)}.
% ARC - OK, for this I am not sure if saying "25% positions" is the most accurate
% term. I think of the % about being a proportion of the span. The window does
% not span 25% of the total chromsoome. Instead, it starts one quarter of the way.
% I do admit that the two can be similarly interpreted which makes this confusing.
% One suggestion here is that you change the text to say, e.g., "three evenly-spaced
% 100 kbp windows, each spanning 10\% of the chromosome length". You don't mention
% the positions and percentages here. Instead, you have a supplemental figure that
% shows how they are organized. Something like you have in Fig 2 but only showing
% the architecture in more detail, which a dedicated grid and drawn to scale.
% Happy to chat more about this.
In the third, QTLs were split across five evenly-spaced windows of the same size, % 100 kb? total span?
\sout{centered at approximately bases 166,667, 333,333, 500,000, 666,667, and 833,333
(the 17\%, 33\%, 50\%, 67\%, and 83\% positions)}.
% ARC - Same here, "split across five evenly spaced 100 kbp windows (Fig SX)",
% removing the number and percentages, referring to the Sup figure instead.
...
we applied SHapley Additive exPlanations (SHAP) using the Python
% ARC - See my comment above about moving the full name for SHAP
\texttt{SHAP} package (\texttt{v.$\geq$0.47.0}) \citep{lundberg_shap_2017},
...
% ARC - I know it is short, but maybe having this with its own
% \paragraph{Top-candidate method} header
In parallel,
...
after filtering. % ARC - this might be in the publication, but info for filtering? I am curious about the missing data proportions since the data processing steps involves an imputation step. This could be as simple as "removing variant with more than X% missing genotypes".
The \dfir genome is large and incompletely
assembled ($\sim$16 Gb; \cite{neale_douglas-fir_2017}), % ARC - 16 Gb?! Nope!
...
% Moving this for readability in the tex doc. No changes to actual text.
...
\begin{figure}[H]
    ...
    mean absolute SHAP scores across the simulated genomic element for neutral sites % Why not "the simulated chromosome"?
...
shown as violin plots. % I removed text here since it is in the caption
Thus,
...
Under genetic architecture 1,
SHAP achieved a pooled AUC-PR of 0.173, % Is this number explicitly in Fig 4? For GA1 I see ELxSHAP=0.21. Maybe this is not the right number?
...
of adaptive loci,
achieving a pooled AUC-PR of 0.843 % OK, this one matches the fig
...
\begin{figure}[H]
    ...
    % One comment here about the axis labels. I think having
    % the "recall" in the X-axis at the very bottom is a bit
    % confusing, since it only applies to the X-axis of the 
    % top row. The X-axis for the bottom row are the
    % categorical values for the three analyses. Then you 
    % have the architecture diagrams on the X too, adding to
    % the confusion. I think you can move the diagrams to the
    % top (next to the a, b, ...), since it applies to all
    % the panels in the column. The "Recall" should be only 
    % for the top row. The X-axis label for the bottom row 
    % could then be "method" or something like that.
...
Although the array itself carries no genomic coordinates,
...
how hard the prediction problem is depends on the biology
...
(we return to this under Strengths and limitations).
...
So \ecolocator is not simply handing the same spatially structured loci
