...
\citep{lundberg_shap_2017}, %ADK -- didn't you use a software package for this? If so, cite it here.
...
\begin{figure}[H] % or [h] if you prefer floating
    ...
    we show Longitude ($R^2$=0.76), Latitude ($R^2$= 0.83)
    , Mean Cold Month Temperature (MCMT)($R^2$= 0.73), Summer Heat Moisture Index (SHM)
    ($R^2$= 0.49), and Mean Summer Precipitation (MSP) ($R^2$= 0.52). \textbf{Panels f-g.}
...
values of 0.84 and 0.76 respectively (\Cref{fig:dfir-results}, \Cref{tab:supptable1}). Combined latitude
...
Mean Cold Month Temperature (MCMT) showed the highest predicted $R^2$ of 0.73, with May to September
Precipitation (MSP) ($R^2$=0.52) and summer heat:moisture index (SHM) ($R^2$=0.49)
...
The highest-attribution SNPs in \Cref{fig:shap-manhattan-dfir} are candidates for future investigation, 
although we are not yet able to cross-reference them against gene models or functional annotation.
...
(Lon. $R^2$=0.76, Lat. $R^2$=0.83, 
MCMT $R^2$=0.73, MSP $R^2$=0.52 , 
SHM $R^2$=0.49)---despite a documented low 
...
are the same ones flagged for longitude (\Cref{fig:shap-manhattan-dfir}),
