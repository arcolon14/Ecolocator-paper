...
to identify the conditions that yeild optimal performance,
...
a member of the Pine family that has it's origins in the Paleocene \citep{hermann_genus_1985},
and that has a natural range spanning extensive climatic 
gradients across the North American Pacific coast from California
to British Columbia. While most populations show low genetic differentiation,
high gene flow and high outcrossing rates \citep{slavov_pollen_2005},
Douglas-fir populations still exhibit strong signatures of local adaptation, as adaptive
quanitative pehnotypes track climatic features like winter cold 
temperatures and summer aridity \citep{stclair_genecology_2005}.
...
is central to understanding (mal)adaptation \citep{stclair_1912_2020},
...
EcoLocator complements long-standing traditional knowledge on seed sourcing
(such as 'seed zones' \citep{randall_forest_1996}), and expands management
options that are essential for predicting individual tree and forest contributions
to productivity and resilience to climate change.
...
two heads — one predicting 2D geographic coordinates, one predicting 
...
weights (both defaulting to 1.0, see \textbf{Supplemental Figure 1.}). 
...
\item (\textbf{Optional}) Evaluate model performance using \texttt{ecolocator loo}, 
...
we use forward-in-time population genetic simulations in SLiM \texttt{v.4.2.2} \citep{haller2023slim4} .
...
calculated from each replicate within each regime (see \textbf{Supplemental Figure 4.}).
...
performed best when dispersal was constrained (d = 0.1, Lon. R\textsuperscript{2}=0.968, Lat. R\textsuperscript{2}=0.967, Env. R\textsuperscript{2}=0.769)(ref. Figure 2, panel a.).
As dispersal increased,
EcoLocator's ability to predict both geographic location
and environmental covariates declined 
(d=0.5, Lon. R\textsuperscript{2}=0.815, Lat. R\textsuperscript{2}=0.814, Env. R\textsuperscript{2}=0.336)(ref. Figure 2, panel a.).
...
Location prediction accuracy similarly decling with increasing dispersal
likely reflects reduced isolation by distance
...
affect location prediction (\(\sigma_K\)=0.3, Lon. R\textsuperscript{2}=0.917, Lat. R\textsuperscript{2}=0.916; \(\sigma_K\)=0.5, Lon. R\textsuperscript{2}=0.911, Lat. R\textsuperscript{2}=0.911; \(\sigma_K\)=0.7, Lon. R\textsuperscript{2}=0.906, Lat. R\textsuperscript{2}=0.907),
...
prediction accuracy of the environmental variable (\(\sigma_K\)=0.3, Env. R\textsuperscript{2}=0.609, \(\sigma_K\)=0.5, Env. R\textsuperscript{2}=0.561, \(\sigma_K\)=0.7, Env. R\textsuperscript{2}=0.0.421)
...
EcoLocator's location prediction accuracy remained relatively unchanged (GA1 - Lon. R\textsuperscript{2}=0.967, Lat. R\textsuperscript{2}=0.967; GA2 - Lon. R\textsuperscript{2}=0.967, Lat. R\textsuperscript{2}=0.967; GA3 - Lon. R\textsuperscript{2}=0.968, Lat. R\textsuperscript{2}=0.968),
and environmental covariate prediction accuracy was largely similar (GA1 - Env. R\textsuperscript{2}=0.759; GA2 - Env. R\textsuperscript{2}=0.771; GA3 - Env. R\textsuperscript{2}=0.787),
...
These results suggest that EcoLocator's performance
...
selection model,
and genetic architecture may shape prediction difficulty.
...
interact in more complex ways. To this end,
...
have shown that trees show maladaptive responses
...
and they were later expanded to include state and private lands
...
Our empirical dataset consisted of 348 unrelated coastal Douglas-fir individuals
...
for diameter, height, and stem form. Since Douglas-fir exhibits low self-fertility, 
seeds derived from maternal parents are likely pollinated by multiple nearby 
pollen donors, producing half-sib families instead of full-sibs or selfs. 
Given the limited movement of Douglas-fir pollen, 
paternal contributions are expected to be restricted to nearby trees ($<$400 m), 
making the maternal source environment a reasonable proxy for the source of both parents.

Leaf-derived DNA from individual trees was genotyped using the 58K Douglas-fir 
...
To represent the climate conditions of maternal trees, 
...
\begin{figure}[H] % or [h] if you prefer floating
    ...
    \caption{Douglas fir case study results. }
    \label{fig:5}
\end{figure}

%RC NOTE: Will send scripts & data so that figure can be finalized 6/23

\subsubsection*{Prediction accuracy}

EcoLocator predicted source latitudes and longitudes with R\textsuperscript{2}
values of 0.84 and 0.76 respectively ((Figure~\ref{fig:4}), Supplemental Table 1.). Combined latitude
and longitude for each sample gave predicted geographic origins that showed a median error of 51.6km (see \textbf{Supplemental Figure 6.}).
Minimum errors were as low as 1.2km, while maximum errors were as high as 646km. Errors
greater than 200km appeared as unusual outliers and were not associated with obvious
geographic features (e.g. edge of range; clustered locations). Label errors or failed predictions are likely
underlying sources for these unuaually large errors. 

We evaluated climate predictions using two approaches: using direct climate predictions
from Ecolocator; or indirect climate predictions from Ecolocator geographic predictions
combined with climate values estimated from ClimateNA. Direct estimates from Ecolocator 
showed higher accuracy than translating locations 
to climate via ClimateNA variables (ref. \textbf{Supplemental Table 1.}), indicating Ecolocator's ability to
identify climate-related covariance in the geographic-genomic matrix, and ability to use
this covariance to improve local climate estimates. Among the three modeled climate variables, 
Mean Cold Month Temperature (MCMT) showed the highest predicted R\textsuperscript{2} of 0.73, with May to September
Precipitation (MSP) (R\textsuperscript{2}=0.52) and summer heart:moisture index (SHM) (R\textsuperscript{2}=0.49)
showing lower and comparable prediction accuracies. These correlations are consistent 
with climate patterns in the Pacific Northwestern U.S., as monthly minimum temperatures
are vastly easier to model than monthly minimum temperatures due to complex interactions
between topography and aspect with local precipitation and aridity \citep{frankson_oregon_2022}.
...
we compared EcoLocator-predicted values to climate ranges 
...
Douglas-fir populations show broader climate tolerance than are reflected in first-generation zones.  
...
rather than the original 500-foot elevation bands \citep{shalev_zone_2026}.
This allows us to evaluate results in the context of first-generation seed 
zone boundaries (which remain widely used in forest management), while making comparisons to climate 
...
using two approaches: (a) using the map-based seed zone as fixed constraint; and 
(b) using individual seed zone climate ranges to define maximum climate transfers (focal point-based). 

In the first comparison, Ecolocator climate predictions were evaluated for whether they fell within 
a percentile-defined range for each tree's source seed zone and elevation band.  Specifically, 
we compared predicted climate values to the 100th percentile range (defined by the minimum and maximum values) 
and the 98th percentile range (defined by the 1st and 99th percentiles). 
This map-based approach treats zones as fixed, and it can lead to “non-matches” due to the 
arbitrary nature of seed zone boundaries, even when predictions are near-perfect. 
Using this approach, we find that 66.1\%
...
\begin{table}[ht]
  ...
  \begin{tabular}{lcccc}
  \hline
   & \multicolumn{2}{c}{EcoLocator} & \multicolumn{2}{c}{Locator+ClimateNA} \\
  \cline{2-3} \cline{4-5}
...
In the second comparison, Ecolocator MCMT predictions were evaluated 
relative to the range of climate variation within the source seed 
zone and the range of climate variation between known and predicted 
values. For example, if a source seed zone--elevation band shows a 
MCMT 100th percentile range of 2°C, an Ecolocator MCMT prediction 
within 2°C of the true value fall within the range of transfers 
allowed within seed zones. This type of ‘focal point’ match 
considers the magnitude of climate difference between the known 
and predicted values at a site, and the climate range of that 
specific seed zone, rather than arbitrary geographic boundaries. 
Using this approach, we find that 93.1\% of the MCMT predictions 
...
(Table~\ref{tab:table2}). Predictions for MAP and SHM showed higher accuracies, 
...
(REF FIG SEED ZONE). In our sample, trees from coastal seed 
zones in Oregon and Washington showed higher accuracy predictions 
for climate variables (MCMT, MSP, SHM) than samples from the Cascade 
Range. Seed zones with highest prediction errors (e.g., 90, 473, 493, 501, 652, and 661; REF MAP IN FIG 5) 
are primarily located near the crest of the Cascade Range, 
which is a natural boundary that divides watersheds and climates, 
and a region with higher climatic complexity than coastal regions. 
Coincidentally, these specific zones also have small sample sizes 
(1 – 2 trees per seed zone), and this may account for their 
unusually high mean error.

%RC: This is correct, but the zone of introgression is limited to east of the Cascade crest in Washington (no evidence of introgression in Oregon pops east of the crest; best ref is StClair 2005).  
%I favor the ‘difficult-to-model’ complexity of the Cascades, coupled with transcription/pre-GPS inaccuracies. 
%While introgression doesn’t seem to line up with error, we might still want to talk about this!  I think it’s interesting that the region showing greatest levels of introgression (SZ’s 232, 402, 621, 622, 631, 641) coincide with regions showing some of the lowest prediction errors. These seed zones show mean deviations from true values around the range of 0.6 to 0.8 degrees MCMT, which is pretty low.
%This means that ECOLOCATOR can accurately map populations that are genetically discontinuous, even if they have unusually climate values (these seed zones have the lowest MCMT values)
...
under historical, current, and projected future climates.
Seed transfer guidelines have been used on U.S. Forest Service lands
for over a half century,
but "off-site'' plantings were common prior to the 1940s,
and widespread restoration efforts with non-local materials
have resulted in well-documented cases of maladaptation.
These plantings have the potential to add new knowledge
about the growth and fitness of provenances planted in novel environments,
but only if their sources can be reliably determined.
Similarly,
forest tree provenance tests established outside native ranges
often identify high-performing genotypes,
but lost tree tags or incomplete records can limit breeding advances
when the source environments of those genotypes are unknown,
making it difficult to identify new germplasm from similar climates.
Efforts to combat illegal logging also increasingly rely
...
could help discriminate among geographically similar
but climatically distinct sources.
Finally,
...
reflecting adaptive differences that would be missed
...
For example,
the original first-generation seed zone--elevation bands
in Oregon and Washington span approximately
1.5\textdegree C in MCMT across the 98th percentile range,
whereas current second-generation seed zones span approximately
2.5\textdegree C in MCMT across the same percentile range [CITE].
Our results suggest that EcoLocator predictions often fall
within the climate tolerance thresholds that forest managers already use
to define locally adapted reforestation material for Douglas-fir.
Importantly,
this level of accuracy was achieved using a modestly sized dataset
of 348 trees and approximately 18,000 SNPs
without known genomic coordinates.
Although additional sampling is not evaluated here,
previous work with Locator suggests that geographic prediction accuracy
can improve with increased sampling density and,
to a lesser extent,
increased genomic sampling \citep{battey_predicting_2020}.
Similar improvements may be possible for EcoLocator,
particularly in regions where current predictions are limited
by sparse sampling or complex environmental gradients.
...
seed genotypes served as proxies for the sampled maternal source trees
and their origins.
However,
seed genotypes include both maternal and paternal contributions,
and each seed tree may be pollinated by multiple nearby pollen donors,
...
but this remains a source of uncertainty when assigning each genotype
...
In this dataset,
higher prediction errors were concentrated in parts of the Cascade Range,
where several non-mutually exclusive factors may reduce prediction accuracy.
First,
steep elevational gradients near the Cascade crest generate sharp climate transitions
that may be difficult to model,
...
Estimation,
rounding,
and digitization errors may therefore introduce
...
(\textit{Pseudotsuga menziesii} var.\ \textit{glauca}) [CITE],
...
These patterns suggest that prediction errors are not only statistical residuals,
but can also point to biologically meaningful features of the study system,
including ancestry transitions,
complex topography,
and uncertainty in historical source records.
...
between the selected window and distant sites (ref \textbf{Supplemental Figure 2., panel a.}).
...
(ref \textbf{Supplemental Figure 2., panel b.}).
...
(ref \textbf{Supplemental Figure 3.}).
For example,
