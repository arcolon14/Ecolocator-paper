...
\citep{endler_geographic_1977, clausen_experimental_1948, kawecki_conceptual_2004}.
...
\sbt{not married to this, just giving a try at framing differently.}
...
We incorporate a downstream feature attribution methods (SHAP \citep{lundberg_shap_2017})
...
\sbt{We will probabky be asked to stick to standard section style, but maybe put this as a subsection of the results?}
...
experiencing local adaption of a quantitative trait 
that evolved to track a single continuously varying environmental axis.
...
simulation were borrowed from  \citet{TODO: cite Chevy 2025},
but are briefly described here from completion.
...
and neutral background variation separately. \sbt{though don't we only evaluate using neutral variations?}
...
/sbt{does this apply to both simulations or just the stepping-stone? Burn-in wasnt' mentioned for above sims}
...
Location prediction accuracy similarly declining with increasing dispersal
...
Env. R\textsuperscript{2}=0.561, \(\sigma_K\)=0.7, Env. R\textsuperscript{2}=0.0.421)
...
(\textit{Pseudotsuga menziesii} var.\ \textit{glauca}) \cite{TODO},
