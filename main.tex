...
Our empirical dataset consisted of open-pollenated seedlings from 
...
pollen donors. While the pollen pool is heterogenous, pollen and seed movement in Douglas-fir
...
minimum errors as low as 1.2km and maximum errors were as high as 646km. Errors
...
to climate via ClimateNA variables (ref. \Cref{tab:supptable1}), highlighting \ecolocator's ability to
identify climate-related covariance in the geographic-genomic matrix, and ability to use
the covariance to improve local climate estimates. Among the three modeled climate variables, 
...
(a) a "fixed-zone" approach where map-based seed zones were treated as fixed constraint; and 
(b) a "focal point" based approach where the climate range of the local seed zone  was used to 
...
zones in Oregon and Washington (e.g., zones 031, 041, 051, 061) showed significantly lower predictions
...
lower predicition errors for MSP and SHM than did Coastal or Cascade Crest trees. The independence
of these patterns to specific geographic locations suggests that they are attributable to some combination
...
whether region-specific anomalies are intractale or whether predictions can be improved by denser 
training data.  
...
and as well as introduced species trials. 
These unintended ‘local adaptation experiments’ provide clear examples of 
maladaptative responses, but they are usually associated with a poor 
...
sources where additonal material can be selected.
Incomplete records or missing tree identities limit the utility of these tests
by making it difficult to identify similar climates to identify pre-adapted seed for screening
and breeding.
