...
used to benchmark the Weighted-Z Analysis (\texttt{WZA}),
...
we applied SHapley Additive exPlanations (SHAP) \texttt{v.$\geq$0.47.0}
...
calculated from each replicate within each regime (see \Cref{fig:local-adaptation}).
...
affect location prediction (\(\sigma_K\)=0.3, Lon. $R^2$=0.917, 
...
(\(\sigma_K\)=0.3, Env. $R^2$=0.609, \(\sigma_K\)=0.5,
Env. $R^2$=0.561, \(\sigma_K\)=0.7, Env. $R^2$=0.421).
...
These results demonstrate that \ecolocator can recover
...
the full simulation $R^2$ results in \Cref{tab:supptable2}.
...
Mean Cold Month Temperature (MCMT) showed the highest predicted $R^2$ of 0.72, with May to September
...
drop to 87.1\% of predictions when using the 98th percentile range 
...
and shared substantial genomic signal with longitude. In contrast, 
...
SHM additionally included candidates associated with heat-shock, mitochondrial stress, 
and redox-response pathways. Although many high-attribution loci were 
...
The Douglas-fir case study extends these findings to a natural 
system where the ground truth is not fully known. 
\ecolocator's predictions of geographic origin and 
climate variables in coastal Douglas-fir are consistent with the 
...
Feature attribution provides additional resolution into how that signal 
is distributed across prediction targets. 
High-attribution SNP sets showed the greatest overlap between targets 
representing closely related geographic or environmental gradients, 
while more distinct targets relied on increasingly different sets of 
genomic features. Functional annotations among prioritized loci also 
corresponded to biological processes relevant to seasonal regulation, 
temperature acclimation, and water-stress response. 
The combination of predictive performance, structured feature overlap, 
and biologically interpretable annotations indicates that \ecolocator 
is recovering both shared and target-specific components of genomic 
variation associated with geography and environment in Douglas-fir, 
while the evolutionary causes of those associations remain subject 
to the limitations of empirical genotype--environment data discussed below.
...
for identifying candidate adaptive loci}
...
where a recombination rate of ($10^{-8}$)
within a 1 Mb genomic element generated extensive LD
between the selected window and distant sites (\Cref{fig:ld-decay-low-recomb}).
Increasing recombination to ($10^{-6}$)
...
The Douglas-fir results nevertheless provide additional information 
...
cessation in coast Douglas-fir \citep{ford_photoperiod_2017}.
...
and stress signaling in Douglas-fir \citep{delatorre_polygenic_2021}. 
...
Douglas-fir has similarly demonstrated climate-related genetic variation in drought 
...
genotype--environment signal captured by \ecolocator in Douglas-fir. 
...
\begin{figure}[H]
    ...
    \caption{\textbf{AUC-PR of Ecolocator/SHAP, \texttt{WZA}, and Top-Candidate GEA Methods Across Two Dispersal/Selection Regimes.}
    ...
    regime (dispersal=0.5, std.\ dev.\ of fitness function=0.7). Ecolocator/SHAP outperforms \texttt{WZA} and
...
\begin{table}[ht]
  \centering
  \caption{\textbf{$R^2$ values for \ecolocator climate variable predictions versus ClimateNA modelled values from predicted locations.}}
...
\begin{table}[ht]
  ...
  \ecolocator prediction targets in coastal Douglas-fir.}}
